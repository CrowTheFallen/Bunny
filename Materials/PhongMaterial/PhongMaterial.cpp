	...
	MaterialGL(name)
{
	...
	l_P = glGetUniformLocation(vp->getId(), "PosLum");
	l_D = glGetUniformLocation(vp->getId(), "DirectionLum");
...
void PhongMaterial::animate(Node* o, const float elapsedTime)
{
	...
	****************************************************/
	...
	Node* lum = scene->getNode("Lumiere");
	glm::vec3 p = lum->frame()->convertPtTo(glm::vec3(0,0,0), o->frame());
	glProgramUniform3fv(vp->getId(), l_P,1, glm::value_ptr(p));
	...
	glProgramUniform3fv(vp->getId(), l_D, 1, glm::value_ptr(spec));
